...
import (
	...
	"github.com/zergwangj/micro/srv/users/handler"
	...
	"github.com/zergwangj/micro/srv/users/subscriber"
	"github.com/micro/go-plugins/broker/kafka"
	broker2 "github.com/micro/go-micro/broker"
)

const serviceName = "zergwangj.srv.users"
...
func main() {
	...
	defer db.Close()
	pub := micro.NewPublisher("CreateUser", service.Client())
	users.RegisterUsersHandler(service.Server(), &handler.Users{
		Db: 	db,
		Conf:	conf,
		Pub:	pub,
	})
	...
	micro.RegisterSubscriber("CreateUser", service.Server(), new(subscriber.Users))
